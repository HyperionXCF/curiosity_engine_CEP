...
from security import create_refresh_token, decode_refresh_token, hash_password , verify_password, create_access_token, decode_access_token
...
def register(request: models.RegisterRequest, db: Session = Depends(init_db)):
    ...
    existing_user = db.query(database_models.User).filter(
        database_models.User.email == request.email
    ).first()
...
level_description = {1: "basic explanation", 2: "intermediate explanation", 3: "in-depth explanation"}


@app.post("/start-learning")
def start_learning(user_input: str, token: str = Depends(oauth2_scheme), db: Session = Depends(init_db)):
    try:
        # Decode token
        ...
        topic = extract_topic_from_user_input(user_input)
        
        # Step 2: Check if a session for this topic exists
        existing_session = db.query(database_models.Session)\
        ...
        if existing_session:
            ...
            interest = db.query(database_models.Interest)\
            ...
            if interest:
                ...
                last_session = db.query(database_models.Session)\
                                .filter(database_models.Session.user_id == user.id)\
                                .filter(database_models.Session.topic == topic)\
                                .order_by(database_models.Session.session_start.desc())\
                                .first()
                if last_session:
                    days_since_last = (datetime.now(timezone.utc) - last_session.session_start).days
        ...
        history = db.query(database_models.History).filter(database_models.History.user_id == user.id).order_by(database_models.History.searched_at.desc()).all()
        sessions = db.query(database_models.Session).filter(database_models.Session.user_id == user.id).order_by(database_models.Session.session_start.desc()).all()
        interests = db.query(database_models.Interest).filter(database_models.Interest.user_id == user.id).all()


        history_data = [{"topic": h.topic, "user_question": h.user_question,
                        "ai_answer": h.ai_answer, "ai_counter_question": h.ai_counter_question}
                        for h in history]

        sessions_data = [{"topic": s.topic, "current_level": s.current_level, "session_start": s.session_start}
                        for s in sessions]
        ...
        personalized_data = {
            ...
            "sessions": [{"topic": s.topic, "started_at": s.session_start} for s in sessions],
            "interests": [i.topic for i in interests]
        }

        # calling our ai action agent for socratic learning
        ai_output = send_prompt_to_llm(socratic_prompt) # this comprise of both main answer and follow-ups
